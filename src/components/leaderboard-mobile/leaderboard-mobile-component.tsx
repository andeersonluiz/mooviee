...
const LeaderboardMobileComponent = () => {
  ...
  const [mostPopular, setMostPopular] = useState<Leaderboarder[] | null>(null);
  const [mostRated, setMostRated] = useState<Leaderboarder[] | null>(null);
  const [nowPlaying, setNowPlaying] = useState<Leaderboarder[] | null>(null);

  useEffect(() => {
    const fetchData = async () => {
      const dataMostPopularPromise = context!.getLeaderboardUseCase.execute(t('language'), 0);
      const dataMostRatedPromise = context!.getLeaderboardUseCase.execute(t('language'), 1);
      const dataNowPlayingPromise = context!.getLeaderboardUseCase.execute(t('language'), 2);
      const [dataMostPopular, dataMostRated, dataNowPlaying] = await Promise.all([
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <p className={`content-center py-8 text-center text-2xl font-bold text-white`}>
            {leaderboardTranslation('title')}
          </p>
          <hr className='mx-8'></hr>
          <div className='pt-8'>
            <HeaderMediaList title={leaderboardTranslation('mostPopular')} viewAll={() => null} />
          </div>
          <ListMediaLeadboarder data={mostPopular} />

          <HeaderMediaList title={leaderboardTranslation('mostRated')} viewAll={() => null} />
          <ListMediaLeadboarder data={mostRated} />

          <HeaderMediaList title={leaderboardTranslation('upcoming')} viewAll={() => null} />
          <ListMediaLeadboarder data={nowPlaying} isUpcoming={true} />
        </>
      )}
    </>
  );
